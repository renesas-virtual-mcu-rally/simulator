...
void mcu_competitor_list(struct board_data_t *board_data) {
  ...
  fclose(fp);
  for (int i = 0; i < nb; i++) {
    for (int j = 0; j < 3; j++) board_data[i].points[j] = 0;
  }
}
...
void mcu_display_clear(WbDeviceTag display) {
  ...
  wb_display_fill_rectangle(display, 0, 0, wb_display_get_width(display),
                            wb_display_get_height(display));
  wb_display_set_color(display, 0x0000FF);
}

void mcu_display_timings(WbDeviceTag display, long current_lap_time,
                         double *laptimes) {
  char str[100];
...
void mcu_init_friction(char *name, double value) {
  ...
  WbFieldRef root_children_field =
      wb_supervisor_node_get_field(root_node, "children");
  WbNodeRef world_node =
      wb_supervisor_field_get_mf_node(root_children_field, 0);
  WbFieldRef contact_props_field =
      wb_supervisor_node_get_field(world_node, "contactProperties");
  int contactPropertyCount = wb_supervisor_field_get_count(contact_props_field);
  for (int i = 0; i < contactPropertyCount; i++) {
    WbNodeRef contact_props_node =
        wb_supervisor_field_get_mf_node(contact_props_field, i);
    WbFieldRef name_field =
        wb_supervisor_node_get_field(contact_props_node, "material2");
    if (strcmp(wb_supervisor_field_get_sf_string(name_field), name) == 0) {
      WbFieldRef cf_field =
          wb_supervisor_node_get_field(contact_props_node, "coulombFriction");
...
void mcu_camera_mounted() {
  ...
  WbNodeRef viewpoint_node =
      wb_supervisor_field_get_mf_node(root_children_field, 1);
  WbFieldRef follow_field =
      wb_supervisor_node_get_field(viewpoint_node, "follow");
  wb_supervisor_field_set_sf_string(follow_field, "robot");
  WbFieldRef follow_type_field =
      wb_supervisor_node_get_field(viewpoint_node, "followType");
  wb_supervisor_field_set_sf_string(follow_type_field, "Mounted Shot");
  WbFieldRef orientation_field =
...
void mcu_camera_pan_tilt(double waypoint[2]) {
  ...
  WbFieldRef position_field =
      wb_supervisor_node_get_field(viewpoint_node, "position");
  double position[3] = {waypoint[0] + 0.03 * (rand() % 100 - 50),
                        1.0 + (rand() % 100) / 50.0,
                        waypoint[1] + 0.03 * (rand() % 100 - 50)};
  wb_supervisor_field_set_sf_vec3f(position_field, position);
}
...
void mcu_leaderboard_display_current(WbNodeRef robot_node, char *data) {
  ...
  WbFieldRef country_field = wb_supervisor_node_get_field(robot_node, "country");
  strcat(display_string, " (");
  strcat(display_string, wb_supervisor_field_get_sf_string(country_field));
  strcat(display_string, ")");
  wb_supervisor_set_label(0, display_string, 0.002, 0.002, 0.1, 0x000000, 0,
                          "Arial Black");
  wb_supervisor_set_label(1, display_string, 0, 0, 0.1, 0xffffff, 0,
                          "Arial Black");
  wb_supervisor_set_label(2, data, 0.002, 0.852, 0.1, 0x000000, 0,
                          "Arial Black");
  wb_supervisor_set_label(3, data, 0, 0.85, 0.1, 0xffffff, 0, "Arial Black");
}

void mcu_leaderboard_display_leaderboard(struct board_data_t *sorted,
                                         int competitor_current,
                                         int current_track) {
  char display_string[2000];
  sprintf(display_string,
          "Pos. Team          Robot         Ctry  Laps  Total time  Best lap "
          " Q.  S.  F.  Total\n");
  for (int i = 0; i < competitor_current; i++) {
    char temp_string[100];
    sprintf(temp_string,
            "%-2d   %-13.13s %-13.13s %-4.4s %5.2f  %10.2f  %8.2f  %2d  %2d  "
            "%2d      %2d\n",
            i + 1, sorted[i].team_name, sorted[i].robot_name, sorted[i].country,
            sorted[i].laps[current_track], sorted[i].times[current_track],
            sorted[i].best_laps[current_track], sorted[i].points[0],
            sorted[i].points[1], sorted[i].points[2],
            sorted[i].points[0] + sorted[i].points[1] + sorted[i].points[2]);
  ...
  wb_supervisor_set_label(4, display_string, 0.021, 0.101, 0.06, 0x000000, 0,
                          "Lucida Console");
  wb_supervisor_set_label(5, display_string, 0.02, 0.1, 0.06, 0xffffff, 0,
                          "Lucida Console");
}
...
int compare_points(const void *a, const void *b) {
  struct board_data_t *da = (struct board_data_t *)a;
  struct board_data_t *db = (struct board_data_t *)b;
  if (da->points[0] + da->points[1] + da->points[2] ==
      db->points[0] + db->points[1] + db->points[2]) {
  ...
  } else if (da->points[0] + da->points[1] + da->points[2] <
             db->points[0] + db->points[1] + db->points[2])
    return 1;
  else
    return -1;
